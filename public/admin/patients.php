...
<main class="flex-1 overflow-auto p-6 lg:p-8">
  ...
  <div class="mb-6">
    <div class="relative max-w-md">
      ...
      <input type="text" id="patient-search" placeholder="Cari nama, email, atau telepon..."
  ...
  <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100" id="patient-tbody">
          ...
          <tr class="hover:bg-slate-50/50 transition-colors patient-row"
              data-name="<?= strtolower(e($p['name'])) ?>"
              data-email="<?= strtolower(e($p['email'])) ?>"
              data-phone="<?= strtolower(e($p['phone'] ?? '')) ?>">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
    ...
    <div class="p-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
      <span id="patient-count">Total <?= $totalPatients ?> pasien terdaftar</span>
    </div>
    <?php endif; ?>
  </div>
...
function filterPatients(query) {
  ...
  if (el) el.textContent = `Menampilkan ${visible} pasien`;
}
</script>
</body>
</html>
