...
foreach ($appointments as $_a) {
    if ($_a['id'] == $selectedAppt) { $selAppt = $_a; break; }
}
?>
<!DOCTYPE html>
<html lang="id">
  ...
  <aside class="w-80 bg-white border-r border-slate-200 flex-col hidden md:flex flex-shrink-0">
    <div class="p-5 border-b border-slate-100">
      <div class="flex items-center gap-3">
        <a href="dashboard.php" class="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-slate-500 hover:text-blue-600 hover:bg-blue-50 transition-colors">
    ...
    <div class="flex-1 overflow-y-auto divide-y divide-slate-50">
      ...
      <a href="?appt=<?= $a['id'] ?>"
        ...
        <div class="w-10 h-10 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center font-bold text-xs flex-shrink-0">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center justify-between gap-2">
            ...
            <?php if ($a['unread'] > 0): ?>
            <span class="bg-blue-600 text-white text-xs font-black rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1"><?= $a['unread'] ?></span>
            <?php endif; ?>
          </div>
  ...
  <main class="flex-1 flex flex-col bg-slate-50">
    ...
    <div class="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-4 shadow-sm">
      ...
      <div class="flex-1">
        ...
        <p class="text-xs text-slate-500"><?= e($selAppt['specialization'] ?? 'Umum') ?> · <?= format_date($selAppt['scheduled_at'], 'd M Y, H:i') ?></p>
      </div>
      <span class="text-xs font-bold px-2.5 py-1 rounded-full <?= ($statusMap[$selAppt['status']] ?? ['','bg-slate-50 text-slate-500'])[1] ?>">
    ...
    <div class="bg-white border-t border-slate-200 px-6 py-4">
      <form id="chat-form" class="flex items-center gap-3">
        ...
        <button type="submit" class="bg-blue-600 text-white w-11 h-11 rounded-xl flex items-center justify-center hover:bg-blue-700 transition-colors shadow-md active:scale-95">
